...
import (
	...
	"github.com/djedjethai/vigilate/internal/config"
	"github.com/djedjethai/vigilate/internal/driver"
...
func (repo *DBRepo) PostHost(w http.ResponseWriter, r *http.Request) {
	...
	h.OS = r.Form.Get("os")
	act, _ := strconv.Atoi(r.Form.Get("active"))
	h.Active = act
	...
	repo.App.Session.Put(r.Context(), "flash", fmt.Sprintf("Changes saved"))
...
// ToggleMonitoring turns monitoring off and on
func (repo *DBRepo) ToggleMonitoring(w http.ResponseWriter, r *http.Request) {
	...
	if enabled == "1" {
		...
		// delete all entries from schedule(should be done, but to be sure)
